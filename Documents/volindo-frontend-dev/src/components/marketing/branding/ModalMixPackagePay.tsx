...
import { purchaseMixPackage } from '@utils/axiosClients';
...
const ModalMixPackagePay = ({
  ...
  const getClientData = (
    ...
    purchaseMixPackage(session?.user?.id_token || '', packageName, body)
      .then(res => {
        clearData();
        if (res?.data?.secret) {
      ...
      .catch(error => {
        ...
        if (error.response.status === 409) {
          ...
          onClose();
          router.replace('/marketing/mix-pack?payment=error', undefined, {
            shallow: true,
          });
        }
      })
        ...
        <MarketModal.Content>
          ...
          {total && service === 'mercado_pago' && (
            <MercadoPagoPayment
              ...
              packageName={'None'}
              createPayment={purchaseMixPackage}
...
export default ModalMixPackagePay;
